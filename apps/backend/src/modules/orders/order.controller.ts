...
const createOrderSchema = z.object({
  tableNumber: z
    .number({ required_error: "شماره میز الزامی است" } as any)
    .int()
...
export const orderController = {
  async create(req: AuthRequest, res: Response) {
    try {
      ...
      if (error.name === "ZodError")
        return sendError(res, "اطلاعات ناقص است", 400);
  ...
  async getActiveByTable(req: AuthRequest, res: Response) {
    try {
      ...
      const order = await orderService.getActiveByTable(
        req.tenantId!,
        tableNumber,
        sessionToken,
      );
      ...
      if (error.message === "SESSION_INVALID")
        return sendError(res, "جلسه منقضی شده", 401);
      return sendError(res, error.message);
    }
  },
  ...
  async updateByCustomer(req: AuthRequest, res: Response) {
    try {
      ...
      if (error.message?.startsWith("EDIT_RESTRICTED"))
        return sendError(
          res,
          "بعد از تایید آشپز فقط می‌توانید آیتم جدید اضافه کنید",
          403,
        );
      if (error.message?.startsWith("EDIT_NOT_ALLOWED"))
        return sendError(res, "سفارش قابل ویرایش نیست", 403);
      return sendError(res, error.message);
    }
  },
