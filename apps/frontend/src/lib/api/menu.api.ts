...
import { ApiResponse, Category, MenuItem } from '@/src/types'
...
export const menuApi = {
  ...
  getCategories: async (slug: string) => {
    const { data } = await apiClient.get<ApiResponse<Category[]>>(`/${slug}/menu/categories`)
    return data.data
  },

  createCategory: async (slug: string, payload: Partial<Category>) => {
    const { data } = await apiClient.post<ApiResponse<Category>>(`/${slug}/menu/categories`, payload)
    return data.data
  },

  updateCategory: async (slug: string, id: string, payload: Partial<Category>) => {
    const { data } = await apiClient.patch<ApiResponse<Category>>(`/${slug}/menu/categories/${id}`, payload)
    return data.data
  },
  ...
  getItems: async (slug: string, categoryId?: string) => {
    const { data } = await apiClient.get<ApiResponse<MenuItem[]>>(`/${slug}/menu/items`, {
      params: { categoryId },
    })
    return data.data
  },
  ...
  createItem: async (slug: string, payload: Partial<MenuItem>) => {
    const { data } = await apiClient.post<ApiResponse<MenuItem>>(`/${slug}/menu/items`, payload)
    return data.data
  },

  updateItem: async (slug: string, id: string, payload: Partial<MenuItem>) => {
    const { data } = await apiClient.patch<ApiResponse<MenuItem>>(`/${slug}/menu/items/${id}`, payload)
    return data.data
  },
